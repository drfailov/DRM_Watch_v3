...
bool disableAlertsTemporarily = false;
...
void ModeAlertSettingsMenuLoop()
{
  if (modeAlertSettingsHourValue < 0)
    modeAlertSettingsHourValue = 23;
  if (modeAlertSettingsHourValue > 23)
    modeAlertSettingsHourValue = 0;
  if (modeAlertSettingsMinuteValue < 0)
    modeAlertSettingsMinuteValue = 59;
  if (modeAlertSettingsMinuteValue > 59)
    modeAlertSettingsMinuteValue = 0;
  if (modeAlertSettingsMelodyValue < 0)
    modeAlertSettingsMelodyValue = getMelodyCount() - 1;
  if (modeAlertSettingsMelodyValue > getMelodyCount() - 1)
    modeAlertSettingsMelodyValue = 0;
...
void ModeAlertSettingsMenuButtonUp()
{
  ...
  if (selected == itemModeAlertSettingsHour)
    modeAlertSettingsHourValue++;
  if (selected == itemModeAlertSettingsMinute)
    modeAlertSettingsMinuteValue++;
  if (selected == itemModeAlertSettingsMelody)
    modeAlertSettingsMelodyValue++;
}
void ModeAlertSettingsMenuButtonDown()
{
  if (!modeAlertSettingsEditMode)
...
void alertLoop()
{
  ...
  if(disableAlertsTemporarily)
    return;
  for (int alertIndex = 0; alertIndex < getAlertsNumber(); alertIndex++)
